...
/** rep: bottom 거리 대비 이 비율 이하면 top, 이상이면 bottom. 사이는 이동 중이라 버린다 */
...
export function labelSample(
  ...
  for (let i = 0; i < s.frames.length; i++) {
    ...
    if (s.kind === 'none') byState.none.push(i);
    else if (s.kind === 'hold') byState.hold.push(i);
  }
  ...
  if (s.kind === 'rep') {
    ...
    const bottomD = base && bottomN ? frameDistance(base, bottomN) : 0;
    if (base && bottomD > 0) {
      for (let i = start; i <= end; i++) {
        const n = norm[i];
        if (!n) continue;
        const r = frameDistance(base, n) / bottomD;
        if (r <= TOP_RATIO) byState.top.push(i);
        else if (r >= BOTTOM_RATIO) byState.bottom.push(i);
      }
    }
  }
